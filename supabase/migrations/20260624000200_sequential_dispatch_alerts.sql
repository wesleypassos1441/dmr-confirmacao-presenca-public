...
set
  status = 'cancelada',
  ultimo_erro = 'Alerta sem resposta cancelado porque a confirmacao inicial ainda nao foi enviada.',
  atualizado_em = now()
from public.escala_colaboradores ec
where fm.escala_colaborador_id = ec.id
  and fm.tipo = 'alerta_sem_resposta'
  and fm.status in ('pendente', 'processando')
  and ec.status_confirmacao = 'sem_resposta'
...
set
  ...
  atualizado_em = now()
where status_confirmacao = 'sem_resposta'
